...
class HTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        self.do_Resp(json.dumps({'opid': data['opid'], 'engine': data['engine'],
                                 'version': 'classfy_v1.0', 'output': digit2str[classify_result]})
                    )
...
if __name__ == "__main__":
    try:
        ...
        # server = ThreadedHttpServer(options.Port)
        server = ThreadedHttpServer(('', int(options.Port)), HTTPHandler)
